...
STATE_DICT = {
    "self_attention.query_transform.weight": torch.tensor([[1.0311, -0.7048, 1.0131, -0.3308],
                                                           [0.5177, 0.3878, -0.5797, -0.1691],
                                                           [-0.5733, 0.5069, -0.4752, -0.4920],
                                                           [0.2704, -0.5628, 0.6793, 0.4405]]),
    "self_attention.key_transform.weight": torch.tensor([[-0.3609, -0.0606,  0.0733,  0.8187],
                                                         [1.4805,  0.3449, -1.4241, -0.1163],
                                                         [0.2176, -0.0467, -1.4335, -0.5665],
                                                         [-0.4253,  0.2625, -1.4391,  0.5214]]),
    "self_attention.value_transform.weight": torch.tensor([[1.0414, -0.3997, -2.2933,  0.4976],
                                                           [-0.4257, -1.3371, -0.1933,  0.6526],
                                                           [-0.3063, -0.3302, -0.9808,  0.1947],
                                                           [-1.6535,  0.6814,  1.4611, -0.3098]]),
    "self_attention.output_transform.weight": torch.tensor([[0.9633, -0.3095,  0.5712,  1.1179],
    ...
    'feature_transformation.linear2.weight': torch.tensor([[-0.1321, -0.8338,  1.9037,  1.3632, -0.5201, -0.1429],
...
def test_layer(layer, input, encoder, encoder_attention_mask, attention_mask, expected):
    ...
    assert torch.allclose(actual, expected, rtol=1e-04)
